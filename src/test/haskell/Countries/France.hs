module Countries.France where

import ParcoursDB.Country
import ParcoursDB.Location

frenchLocation :: String -> Location
frenchLocation loc = Location loc France

-- A
abbeville              = Location "Abbeville" France
agde                   = Location "Agde" France
agen                   = Location "Agen" France
aigurande              = Location "Aigurande" France
aime                   = Location "Aime" France
aixEnProvence          = Location "Aix-en-Provence" France
aixLesBains            = Location "Aix-les-Bains" France
ajaccio                = Location "Ajaccio" France
albertville            = Location "Albertville" France
albi                   = Location "Albi" France
alencon                = Location "Alencon" France
ales                   = Location "Ales" France
alfortville            = Location "Alfortville" France
amiens                 = Location "Amiens" France
ancenis                = Location "Ancenis" France
andorraLaVella         = Location "Andorra la Vella" France
angers                 = Location "Angers" France
angouleme              = Location "Angouleme" France
annecy                 = Location "Annecy" France
annemasse              = Location "Annemasse" France
annonayDavezieux       = Location "Annonay-Davezieux" France
antibes                = Location "Antibes" France
antonyParcDeSceaux     = Location "Antony/Parc de Sceaux" France
arbois                 = Location "Arbois" France
arcEtSenans            = Location "Arc-et-Senans" France
arcachon               = Location "Arcachon" France
arenbergPorteDuHainaut = Location "Arenberg Porte du Hainaut" France
argelesGazost          = Location "Argeles-Gazost" France
argentan               = Location "Argentan" France
argenteuil             = Location "Argenteuil" France
ariege                 = Location "Ariege" France
armentieres            = Location "Armentieres" France
arpajon                = Location "Arpajon" France
arpajonSurCere         = Location "Arpajon-sur-Cere" France
arras                  = Location "Arras" France
aubagne                = Location "Aubagne" France
aubenas                = Location "Aubenas" France
aubisque               = Location "Aubisque" France
aulay                  = Location "Aulay" France
aulnaySousBois         = Location "Aulnay-sous-Bois" France
aurillac               = Location "Aurillac" France
autodromeDeMontlhery   = Location "Autodrome de Montlhéry" France
autrans                = Location "Autrans" France
autun                  = Location "Autun" France
auxerre                = Location "Auxerre" France
avesnesSurHelpe        = Location "Avesnes-sur-Helpe" France
avignon                = Location "Avignon" France
avoriaz                = Location "Avoriaz" France
avranches              = Location "Avranches" France
ax3Domaines            = Location "Ax 3 Domaines" France
axLesThermes           = Location "Ax-les-Thermes" France

-- B
bagneresDeBigorre      = Location "Bagnères-de-Bigorre" France
bagneresDeLuchon       = Location "Bagnères-de-Luchon" France
bagnolesDeL'Orne       = Location "Bagnoles-de-l'Orne" France
ballonD'Alsace         = Location "Ballon d'Alsace" France
barLeDuc               = Location "Bar-le-Duc" France
bastia                 = Location "Bastia" France
bayonne                = Location "Bayonne" France
bazas                  = Location "Bazas" France
beauvais               = Location "Beauvais" France
bedoin                 = Location "Bedoin" France
belfort                = Location "Belfort" France
bellegardeSueValserine = Location "Bellegarde-sue-Valserine" France


belleville :: Location
belleville = frenchLocation "Belleville"

bergerac               = Location "Bergerac" France
besancon               = Location "Besancon" France
besse                  = Location "Besse" France
besseEnChandesse       = Location "Besse-en-Chandesse" France
bethune                = Location "Bethune" France
beziers                = Location "Beziers" France
biarritz               = Location "Biarritz" France
blagnac                = Location "Blagnac" France
blain                  = Location "Blain" France
blayeLesMines          = Location "Blaye-les-Mines" France
blois                  = Location "Blois" France
bobigny                = Location "Bobigny" France
bondy                  = Location "Bondy" France
bonneval               = Location "Bonneval" France
bordeaux               = Location "Bordeaux" France
boulogneBillancourt    = Location "Boulogne-Billancourt" France
boulogneSurMer         = Location "Boulogne-sur-Mer" France
bourgD'Oisans          = Location "Bourg-d'Oisans" France
bourgDePeage           = Location "Bourg-de-Peage" France
bourgEnBresse          = Location "Bourg-en-Bresse" France
bourgLesValence        = Location "Bourg-lès-Valence" France
bourgMadame            = Location "Bourg-Madame" France
bourgSaintAndeol       = Location "Bourg-Saint-Andeol" France
bourgSaintMaurice      = Location "Bourg-Saint-Maurice" France
bourges                = Location "Bourges" France
bourgoin               = Location "Bourgoin" France
bourgoinJallieu        = Location "Bourgoin-Jallieu" France
brest                  = Location "Brest" France
bretignySurOrge        = Location "Bretigny-sur-Orge" France
briancon               = Location "Briançon" France
brignoles              = Location "Brignoles" France
brioude                = Location "Brioude" France
brive                  = Location "Brive" France
briveLaGaillarde       = Location "Brive-la-Gaillarde" France

bron :: Location
bron = frenchLocation "Bron"

-- C
caen = Location "Caen" France
cagnesSurMer = Location "Cagnes-sur-Mer" France
cahors = Location "Cahors" France
calais = Location "Calais" France
calvi = Location "Calvi" France
camboLesBains = Location "Cambo-les-Bains" France
cambrai = Location "Cambrai" France
cancale = Location "Cancale" France
cannes = Location "Cannes" France
capD'Agde = Location "Cap d'Agde" France
capD'Agne = Location "Cap d'Agne" France
capDecouverte = Location "Cap Decouverte" France
capFrehel = Location "Cap Frehel" France
captieux = Location "Captieux" France
carcassonne     = Location "Carcassonne" France
carhaix = Location "Carhaix" France
carmaux = Location "Carmaux" France
carnonPlage = Location "Carnon-Plage" France
carpentras = Location "Carpentras" France
castelnaudary = Location "Castelnaudary" France
castelsarrasin = Location "Castelsarrasin" France
castillonLaBataille = Location "Castillon-la-Bataille" France
castres = Location "Castres" France
cauterets = Location "Cauterets" France
cergyPontoise = Location "Cergy-Pontoise" France
cerilly = Location "Cerilly" France
chablis = Location "Chablis" France
chaletReynard = Location "Chalet Reynard (Mont Ventoux)" France
challans = Location "Challans" France
chalonSurSaone = Location "Chalon-sur-Saone" France
chalonsSurMarne = Location "Chalons-sur-Marne" France
chambery = Location "Chambery" France
chambord = Location "Chambord" France
chamonix = Location "Chamonix" France
champagnole = Location "Champagnole" France
chamrousse = Location "Chamrousse" France
chantilly       = Location "Chantilly" France
chantonnay = Location "Chantonnay" France
charleville = Location "Charleville" France
charlevilleMezieres = Location "Charleville-Mezieres" France
chartres            = Location "Chartres" France
chassieu            = Location "Chassieu" France
chateauThierry      = Location "Chateau-Thierry" France
chateaubriant       = Location "Chateaubriant" France
chateaulin          = Location "Chateaulin" France
chateauroux         = Location "Chateauroux" France
chatelGuyon         = Location "Chatel-Guyon" France
chatellerault       = Location "Chatellerault" France
chatou              = Location "Chatou" France
chaumeil            = Location "Chaumeil" France
chaville            = Location "Chaville" France
chazey              = Location "Chazey-sur-Ain" France
cherbourg = Location "Cherbourg" France
cherbourgEnCotentin = Location "Cherbourg-en-Cotentin" France
cholet              = Location "Cholet" France
chorges             = Location "Chorges" France

ciney :: Location
ciney = frenchLocation "Ciney"

circuitDesEssarts   = Location "Circuit des Essarts" France
circuitDeRouenLesEssarts  = Location "Circuit de Rouen-Les-Essarts" France

circuitDeLaPrairie :: Location
circuitDeLaPrairie = frenchLocation "Circuit de la Prairie"

clermontFerrand     = Location "Clermont-Ferrand" France
cluses              = Location "Cluses" France
cognac              = Location "Cognac" France
colmar              = Location "Colmar" France
colombiers          = Location "Colombiers" France
commercy            = Location "Commercy" France
compiegne           = Location "Compiegne" France
concarneau          = Location "Concarneau" France
corbeilEssonnes     = Location "Corbeil-Essonnes" France
correze             = Location "Correze" France
cosneSurLoire       = Location "Cosne-sur-Loire" France
courchevel          = Location "Courchevel" France
creteil             = Location "Creteil" France
cugnaux             = Location "Cugnaux" France
culoz               = Location "Culoz" France

-- D
damazan :: Location
damazan = frenchLocation "Damazan"

dax              = Location "Dax" France
deauville        = Location "Deauville" France
dieppe           = Location "Dieppe" France
digne            = Location "Digne" France
digneLesBains    = Location "Digne-les-Bains" France
dijon            = Location "Dijon" France
dinan            = Location "Dinan" France
dinard           = Location "Dinard" France
disneylandParis  = Location "Disneyland Paris" France
divonneLesBains  = Location "Divonne-les-Bains" France
dole             = Location "Dole" France
domaineDuRouret  = Location "Domaine du Rouret" France
dordogne         = Location "Dordogne" France
douai            = Location "Douai" France
draguignan       = Location "Draguignan" France
dreux            = Location "Dreux" France
drome            = Location "Drome" France
dunkerque        = Location "Dunkerque" France

-- E
embrun          = Location "Embrun" France
epernay         = Location "Epernay" France
epinal          = Location "Epinal" France
epnaySousSenart = Location "Epnay-sous-Senart" France
espelette       = Location "Espelette" France
etampes         = Location "Etampes" France
eurotunnel      = Location "Eurotunnel" France
evian           = Location "Evian" France
evianLesBains   = Location "Evian-les-Bains" France
evreux          = Location "Evreux" France
evry            = Location "Evry" France
eymet           = Location "Eymet" France

-- F
faverges            = Location "Faverges" France
fecamp              = Location "Fecamp" France
figeac              = Location "Figeac" France
flers               = Location "Flers" France
fleurance           = Location "Fleurance" France
fleurence           = Location "Fleurence" France
foix                = Location "Foix" France
fontRomeuOdeilloVia = Location "Font-Romeu-Odeillo-Via" France
fontaine            = Location "Fontaine" France
fontaineAuPire      = Location "Fontaine-au-Pire" France
fontainebleau       = Location "Fontainebleau" France
fontenayLeComte     = Location "Fontenay-le-Comte" France
fontenaySousBois    = Location "Fontenay-sous-Bois" France
forgesLesEaux       = Location "Forges-les-Eaux" France
fougeres            = Location "Fougeres" France
fougres             = Location "Fougres" France
fromentine          = Location "Fromentine" France
frontenex           = Location "Frontenex" France
frontignanLaPeyrade = Location "Frontignan la Peyrade" France
futuroscope         = Location "Futuroscope" France

-- G
gaillac :: Location
gaillac = frenchLocation "Gaillac"

gap = Location "Gap" France
gerardmer = Location "Gerardmer" France
gerardmerLasMauselaine = Location "Gerardmer las Mauselaine" France
gex = Location "Gex" France
givors = Location "Givors" France
gourette = Location "Gourette" France
gouretteColD'Aubisque = Location "Gourette-Col d'Aubisque" France
granville = Location "Granville" France
grenoble = Location "Grenoble" France
gueret = Location "Gueret" France
gueugnon = Location "Gueugnon" France

guillestre :: Location
guillestre = frenchLocation "Guillestre"

guzetNeige = Location "Guzet-Neige" France

-- H
hautacam = Location "Hautacam" France

hendaye :: Location
hendaye = frenchLocation "Hendaye"

houilles = Location "Houilles" France
hyeres   = Location "Hyeres" France

-- I
ileD'Oleron = Location "Ile d'Oleron" France
isola = Location "Isola" France
isola_2000 = Location "Isola 2000" France
issoire = Location "Issoire" France
issoudun = Location "Issoudun" France

-- J
jausiers = Location "Jausiers" France
joigny = Location "Joigny" France
joinville = Location "Joinville" France
jonzac = Location "Jonzac" France

-- L
l'IsleD'Abeau = Location "L'Isle-d'Abeau" France
l'IsleJourdain = Location "L'Isle-Jourdain" France
laBaule = Location "La Baule" France
laBourboule = Location "La Bourboule" France
laCaverneDuPontD'Arc = Location "La Caverne du Pont-d'Arc" France
laChataigneraie = Location "La Chataigneraie" France
laChatre = Location "La Chatre" France
laDefense = Location "La Defense" France
laFerteSousJouarre = Location "La Ferte-sous-Jouarre" France
laFouillouse               = Location "La Fouillouse" France
laGrandMotte = Location "La Grand-Motte" France
laGrandeMotte = Location "La Grande-Motte" France
laGuercheDeBretagne = Location "La Guerche-de-Bretagne" France
laHaieFouassiere = Location "La Haie-Fouassiere" France
laMongie = Location "La Mongie" France
laMure = Location "La Mure" France
laPierreSaintMartin = Location "La Pierre Saint-Martin" France
laPlagne = Location "La Plagne" France
laPlancheDesBellesFilles = Location "La Planche des Belles Filles" France
laRocheSurYon = Location "La Roche-sur-Yon" France
laRochelle = Location "La Rochelle" France
laRuchere = Location "La Ruchere" France
laTourDuPin = Location "La Tour-du-Pin" France
laToussuire = Location "La Toussuire" France
laToussuireLesSybelles = Location "La Toussuire - Les Sybelles" France
labastideD'Armagnac = Location "Labastide-d'Armagnac" France
lacDeMadine = Location "Lac de Madine" France
lacDePayolle = Location "Lac de Payolle" France
lacDeVassiviere = Location "Lac de Vassiviere" France
lacSaintPoint = Location "Lac Saint-Point" France
laissacSeveracL'Eglise = Location "Laissac-Severac-l'Eglise" France
lamballe = Location "Lamballe" France
lanester = Location "Lanester" France
langon = Location "Langon" France
lannemezan = Location "Lannemezan" France
lannion = Location "Lannion" France
lansEnVercors = Location "Lans-en-Vercors" France
laon = Location "Laon" France
laplume = Location "Laplume" France
laruns = Location "Laruns" France
laval = Location "Laval" France
lavalanet = Location "Lavalanet" France
lavaur = Location "Lavaur" France
lavelanet = Location "Lavelanet" France
leBlanc = Location "Le Blanc" France
leBourgD'Oisans = Location "Le Bourg-d'Oisans" France
leCapD'Agde = Location "Le Cap d'Agde" France
leCreusot = Location "Le Creusot" France
leGrandBornand = Location "Le Grand Bornand" France
leHavre = Location "Le Havre" France
leLioran = Location "Le Lioran" France
leMans = Location "Le Mans" France
leMonetierLesBains = Location "Le Monetier-les-Bains" France
lePerreuxSurMarne = Location "Le Perreux-sur-Marne" France
lePleynet = Location "Le Pleynet" France
lePuy = Location "Le Puy" France
lePuyDuFou = Location "Le Puy du Fou" France
lePuyEnVelay = Location "Le Puy-en-Velay" France
leRevard = Location "Le Revard" France
leTouquet = Location "Le Touquet" France
leTouquetParisPlage = Location "Le Touquet-Paris-Plage" France
leTreport = Location "Le Treport" France
lesArcs = Location "Les Arcs" France
lesDeuxAlpes = Location "Les Deux Alpes" France
lesEchelles = Location "Les Echelles" France
lesEssarts = Location "Les Essarts" France
lesMenuires = Location "Les Menuires" France
lesSablesD'Olonne = Location "Les Sables-d'Olonne" France
levalloisPerret = Location "Levallois-Perret" France
lezatSurLeze = Location "Lezat-sur-Leze" France
lezignanCorbieres = Location "Lezignan-Corbieres" France
libourne = Location "Libourne" France
lievin = Location "Lievin" France
lille = Location "Lille" France
lilleMetropole = Location "Lille Metropole" France
limoges = Location "Limoges" France
limoux = Location "Limoux" France
lisieux = Location "Lisieux" France
livarot = Location "Livarot" France
lodeve = Location "Lodeve" France
longjumeau = Location "Longjumeau" France
longwy = Location "Longwy" France
lonsLeSaunier = Location "Lons-le-Saunier" France
lorient = Location "Lorient" France
loudenvielle = Location "Loudenvielle" France
loudenville = Location "Loudenville" France
loudon = Location "Loudon" France
louhans                    = Location "Louhans-Chateaurenaud" France
lourdes = Location "Lourdes" France
louvroil = Location "Louvroil" France
luchon = Location "Luchon" France
lucon = Location "Lucon" France
lugny = Location "Lugny" France
luneville = Location "Luneville" France
luzArdiden = Location "Luz Ardiden" France
luzSaintSauveur = Location "Luz-Saint-Sauveur" France
lyon = Location "Lyon" France

-- M
machecoul = Location "Machecoul" France
macon = Location "Macon" France
maloLesBains = Location "Malo-les-Bains" France
manosque = Location "Manosque" France
marcoussis = Location "Marcoussis" France
marennes = Location "Marennes" France
marseille              = Location "Marseille" France
martigues = Location "Martigues" France
martingues = Location "Martingues" France
mauberge = Location "Mauberge" France
maubourguetPaysDuValD'Adour = Location "Maubourguet Pays du Val d'Adour" France
mayenne = Location "Mayenne" France
mazamet = Location "Mazamet" France
mazeMontgeoffroy = Location "Maze-Montgeoffroy" France
meaux = Location "Meaux" France
megeve = Location "Megeve" France
melinPlage = Location "Melin-Plage" France
melun = Location "Melun" France
mende                  = Location "Mende" France
menton = Location "Menton" France
merlinPlage = Location "Merlin-Plage" France
mersLesBains = Location "Mers-les-Bains" France
metz = Location "Metz" France
meudon                 = Location "Meudon" France
meyrignacL'Eglise = Location "Meyrignac-l'Eglise" France
millau                 = Location "Millau" France
miramas = Location "Miramas" France
modane = Location "Modane" France
moiransEnMontagne = Location "Moirans-en-Montagne" France
montDeMarsan           = Location "Mont-de-Marsan" France
montDesAlouettes = Location "Mont des Alouettes" France
montSaintMichel = Location "Mont Saint-Michel" France
montaigu = Location "Montaigu" France
montargis = Location "Montargis" France
montauban = Location "Montauban" France
montbeliard = Location "Montbeliard" France
montbrison             = Location "Montbrison" France
montceauLesMines = Location "Montceau-les-Mines" France
montelimar = Location "Montelimar" France
montereauFaultYonne = Location "Montereau-Fault-Yonne" France
montgeron              = Location "Montgeron" France
montlhery = Location "Montlhery" France
montlucon = Location "Montlucon" France
montpellier            = Location "Montpellier" France
montponMenesterol = Location "Montpon-Menesterol" France
montreuil = Location "Montreuil" France
morzine = Location "Morzine" France
morzineAvoriaz = Location "Morzine-Avoriaz" France
mouilleronSaintGermain = Location "Mouilleron-Saint-Germain" France
mourenx                = Location "Mourenx" France
moutiers               = Location "Moûtiers" France
mulhouse               = Location "Mulhouse" France
muret = Location "Muret" France

-- N
nancy = Location "Nancy" France
nanterre = Location "Nanterre" France
nantes = Location "Nantes" France
nantua = Location "Nantua" France
narbonne = Location "Narbonne" France
nay = Location "Nay" France
nemours = Location "Nemours" France
neufchatelEnBray = Location "Neufchatel-en-Bray" France
neuvilleDePoitou = Location "Neuville-de-Poitou" France
nevers = Location "Nevers" France
nice        = Location "Nice" France
nimes = Location "Nimes" France
niort = Location "Niort" France
nogentSurMarne = Location "Nogent-sur-Marne" France
nogentSurOise = Location "Nogent-sur-Oise" France
noirmoutierEnL'Ile :: Location
noirmoutierEnL'Ile = frenchLocation "Noirmoutier-en-l'Île"

noisyLeSec = Location "Noisy-le-Sec" France
nuitsSaintGeorges = Location "Nuits-Saint-Georges" France

-- O
obernai = Location "Obernai" France
olonneSurMer = Location "Olonne-sur-Mer" France
orchies = Location "Orchies" France
orcieres = Location "Orcieres" France
orcieresMerlette = Location "Orcieres-Merlette" France
orleans = Location "Orleans" France
orsonville = Location "Orsonville" France
orthez = Location "Orthez" France

oyonnax :: Location
oyonnax = frenchLocation "Oyonnax"

-- P
palaiseau :: Location
palaiseau = frenchLocation "Palaiseau"

pamiers = Location "Pamiers" France
pantin = Location "Pantin" France
paris = Location "Paris" France
passageDuGois = Location "Passage du Gois" France
pau = Location "Pau" France
pauillac = Location "Pauillac" France
perigueux = Location "Périgueux" France
peronne = Location "Peronne" France
perpignan = Location "Perpignan" France
perrosGuirec = Location "Perros-Guirec" France
peyragudes = Location "Peyragudes" France
peyrehorade = Location "Peyrehorade" France
piauEngaly = Location "Piau-Engaly" France
plaD'Adet = Location "Pla d'Adet" France
plateauDeBeille = Location "Plateau de Beille" France
plateauDeBonascre = Location "Plateau de Bonascre" France
plouay = Location "Plouay" France
plumelec = Location "Plumelec" France
poitiers     = Location "Poitiers" France
pontAudemer = Location "Pont-Audemer" France
pontDeVaux   = Location "Pont-de-Vaux" France
pontL'Eveque = Location "Pont l'Eveque" France
pontarlier = Location "Pontarlier" France
pontchateau = Location "Pontchateau" France
pontoise = Location "Pontoise" France
pornic = Location "Pornic" France
pornichet = Location "Pornichet" France
porrentruy = Location "Porrentruy" France
porteMaillot = Location "Porte Maillot, Paris" France
portoVecchio = Location "Porto-Vecchio" France
praLoup = Location "Pra-Loup" France
prapoutel = Location "Prapoutel" France
privas = Location "Privas" France
provence = Location "Provence" France
puyDeDome    = Location "Puy de Dome" France

-- Q
quimper = Location "Quimper" France

-- R
rambouillet :: Location
rambouillet = frenchLocation "Rambouillet"

redon = Location "Redon" France
regnieDurette = Location "Regnie-Durette" France
reims = Location "Reims" France
renaze = Location "Renaze" France
rennes = Location "Rennes" France
revel = Location "Revel" France
risoul = Location "Risoul" France
roanne = Location "Roanne" France
rochefort = Location "Rochefort" France
rodez = Location "Rodez" France
romansSurIsere = Location "Romans-sur-Isere" France
roquefortSurSoulzon = Location "Roquefort-sur-Soulzon" France
roscoff = Location "Roscoff" France
roubaix = Location "Roubaix" France
rouen = Location "Rouen" France
royan = Location "Royan" France
ruelleSurTourve = Location "Ruelle sur Tourve" France
ruffex = Location "Ruffex" France
rungis = Location "Rungis" France

-- S
saintAmandLesEaux = Location "Saint-Amand-les-Eaux" France
saintAmandMontrond = Location "Saint-Amand-Montrond" France
saintBrieuc = Location "Saint-Brieuc" France
saintDenis = Location "Saint-Denis" France
saintDierD'Auvergne = Location "Saint-Dier-d'Auvergne" France
saintDizier = Location "Saint-Dizier" France
saintEmilion = Location "Saint-Emilion" France

saintÉTienne :: Location
saintÉTienne = frenchLocation "Saint-Étienne"

saintFargeau = Location "Saint-Fargeau" France
saintFlour = Location "Saint-Flour" France
saintFrancoisLongchamp = Location "Saint-François-Longchamp" France
saintGalmier = Location "Saint Galmier" France
saintGaudens = Location "Saint-Gaudens" France
saintGermain           = Location "Saint-Germain" France
saintGermainEnLaye = Location "Saint-Germain-en-Laye" France
saintGervais = Location "Saint-Gervais" France
saintGervaisLesBains = Location "Saint Gervais-les-Bains" France
saintGervaisMontBlanc  = Location "Saint-Gervais Mont Blanc" France
saintGildasDesBois = Location "Saint-Gildas-des-Bois" France
saintGirons = Location "Saint-Girons" France
saintGregoire = Location "Saint Gregoire" France
saintHerblain = Location "Saint-Herblain" France

saintHilaireDuHarcouet :: Location
saintHilaireDuHarcouet = frenchLocation "Saint-Hilaire-du-Harcouët"

saintJeanDeMaurienne = Location "Saint-Jean-de-Maurienne" France
saintJeanDeMonts = Location "Saint-Jean-de-Monts" France
saintJulienEnGenevois = Location "Saint-Julien-en-Genevois" France
saintJust              = Location "Saint-Just-Saint-Rambert" France
saintLaryPlaD'Adet = Location "Saint-Lary Pla d'Adet" France
saintLarySoulan        = Location "Saint-Lary-Soulan (Col de Portet)" France
saintLarySoulanPlaD'Adet = Location "Saint-Lary-Soulan Pla d'Adet" France
saintLeonardDeNoblat = Location "Saint-Leonard-de-Noblat" France
saintLo = Location "Saint-Lo" France
saintMaixentL'Ecole = Location "Saint-Maixent-l'Ecole" France
saintMalo = Location "Saint-Malo" France
saintMartinDeLandelles = Location "Saint-Martin-de-Landelles" France
saintMeenLeGrand = Location "Saint-Meen-le-Grand" France
saintNazaire = Location "Saint-Nazaire" France
saintOmer = Location "Saint-Omer" France
saintOrensDeGameville = Location "Saint-Orens-de-Gameville" France
saintPaulTroisChateaux = Location "Saint-Paul-Trois-Chateaux" France
saintPeeSurNivelle     = Location "Saint-Pee-sur-Nivelle" France
saintPourcainSurSioule = Location "Saint-Pourcain-sur-Sioule" France
saintPriest = Location "Saint-Priest" France
saintQuentin = Location "Saint-Quentin" France
saintQuentinEnYvelines = Location "Saint-Quentin-en-Yvelines" France
saintRaphael = Location "Saint-Raphael" France
saintValeryEnCaux = Location "Saint-Valery-en-Caux" France
sainteFoyLaGrande = Location "Sainte-Foy-la-Grande" France
sainteGenevieveDesBois = Location "Sainte-Genevieve-des-Bois" France
saintes = Location "Saintes" France
saliesDeBearn = Location "Salies-de-Bearn" France
sallanches = Location "Sallanches" France
salonDeProvence = Location "Salon-de-Provence" France
samatan = Location "Samatan" France
santennay = Location "Santennay" France
sarran = Location "Sarran" France
sarrebourg = Location "Sarrebourg" France
sarzeau                = Location "Sarzeau" France
saumur = Location "Saumur" France
sauternes = Location "Sauternes" France
sceaux = Location "Sceaux" France
sedan = Location "Sedan" France
semnoz = Location "Semnoz" France
semurEnAuxois = Location "Semur-en-Auxois" France
senlis = Location "Senlis" France
sens = Location "Sens" France
serreChevalier = Location "Serre Chevalier" France
sete = Location "Sete" France
seurre = Location "Seurre" France
sevres = Location "Sevres" France
sisteron = Location "Sisteron" France
soissons = Location "Soissons" France
stationDesRousses = Location "Station des Rousses" France
strasbourg = Location "Strasbourg" France
superBesse = Location "Super Besse" France
superBesseSancy = Location "Super-Besse Sancy" France
superbagneres = Location "Superbagneres" France
suresnes               = Location "Suresnes, Paris" France

-- T
tallard           = Location "Tallard" France
tarasconSurAriege = Location "Tarascon-sur-Ariege" France
tarbes            = Location "Tarbes" France
thionville        = Location "Thionville" France
thononLesBains    = Location "Thonon-les-Bains" France
tignes            = Location "Tignes" France
tomblaine         = Location "Tomblaine" France
tonnerre          = Location "Tonnerre" France
toulon            = Location "Toulon" France
toulouse          = Location "Toulouse" France
tournus           = Location "Tournus" France
tours             = Location "Tours" France
trelissac         = Location "Trelissac" France
trets             = Location "Trets" France
trieSurBasie      = Location "Trie-sur-Basie" France
trouville         = Location "Trouville" France
troyes            = Location "Troyes" France
tulle             = Location "Tulle" France

-- U
utahBeach = Location "Utah Beach (Sainte-Marie-du-Mont)" France

-- V
vaisonLaRomaine      = Location "Vaison-la-Romaine" France
valD'AranPlaDeBeret  = Location "Val d'Aran/Pla-de-Beret" France
valD'Isere           = Location "Val d'Isere" France
valLouron            = Location "Val Louron" France
valThorens           = Location "Val Thorens" France
valdebloreLaColmiane = Location "Valdeblore La Colmiane" France
valence              = Location "Valence" France
valenceD'Agen        = Location "Valence d'Agen" France
valenciennes         = Location "Valenciennes" France
valloire             = Location "Valloire" France
valmorel             = Location "Valmorel" France
valreas              = Location "Valreas" France
valsLesBains         = Location "Vals-les-Bains" France
vannes               = Location "Vannes" France
vatan                = Location "Vatan" France
vaucluse             = Location "Vaucluse" France
vence                = Location "Vence" France
vercors              = Location "Lan-en-Vercors" France
verdun               = Location "Verdun" France
versailles           = Location "Versailles" France
vesoul               = Location "Vesoul" France
veurey               = Location "Veurey" France
vichy                = Location "Vichy" France
vierzon              = Location "Vierzon" France
villarLesDombes      = Location "Villar-les-Dombes (Parc des Oiseaux)" France
villardDeLans        = Location "Villard-de-Lans" France
villeD'Avray         = Location "Ville-d'Avray" France
villefrancheSurSaone = Location "Villefranche-sur-Saone" France
villeneuveSurLot     = Location "Villeneuve-sur-Lot" France
villersCotterets     = Location "Villers-Cotterets" France
villersSurMer        = Location "Villers-sur-Mer" France
villeurbanne         = Location "Villeurbanne" France
villieMorgon         = Location "Villie-Morgon" France
vire                 = Location "Vire" France
viryChatillon        = Location "Viry-Chatillon" France
vitre                = Location "Vitre" France
vittel               = Location "Vittel" France
vizelle              = Location "Vizelle" France
voreppe              = Location "Voreppe" France

-- W
wasquehal = Location "Wasquehal" France
